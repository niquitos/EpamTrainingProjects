...
namespace Client.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Secret()
        {
            ...
            if(serverResponse.StatusCode==HttpStatusCode.OK && apiResponse.StatusCode == HttpStatusCode.OK)
            {
                return View();
            }
        ...
        private async Task RefreshAccessToken()
        {
            ...
            var request = new HttpRequestMessage(HttpMethod.Post, "https://localhost:44377/oauth/token")
            {
                Content = new FormUrlEncodedContent(requestData)
            };

            var basicCredentials = "username:password";
            var bytesCredentials = Encoding.UTF8.GetBytes(basicCredentials);
            var base64Credentials = Convert.ToBase64String(bytesCredentials);
